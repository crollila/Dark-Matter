...
- ENVIRONMENT SHEETS (9 new 1254x1254 sheets in `SPRITE_SHEETS`, clean `cols:8,rows:8` = 156.75px cells, addressed by grid FRACTION; one cell per tile, never a block): `env_neutral/forest/goblin/fungal/void/frost/infernal/cursed/plague`. VISUAL-ONLY terrain layer — generation/collision/hazards/portals/mobs/loot/stations untouched; the flat-color tile fill remains the fallback. DATA-DRIVEN tables (all in sprites.js, easy to remap one line): `ENV_SHEET_BY_THEME` (theme→sheet), `ENV_ROLE_ASSIGNMENTS` (role→candidate cells: floor/floorAlt/path/wall/wallAlt/specialFloor/hazard/water/smallDecor/largeDecor/ruin) + `ENV_ROLE_OVERRIDES` (per-theme hazard/liquid repoint), `envHazardAssignments` (tile-name→role), `envDecorAssignments` (per-theme sparse smallDecor chance), `biomeEnvThemeMap` (biome id→theme, incl. boss biomes 7-12 + low/mid 13-19), `dungeonEnvThemeMap` (dungeon key→theme, fallback neutral). Helpers: `Sprites.drawEnvTile(theme,role,cx,cy,size,ctx,seed[,opts])` (resolves sheet+role, picks ONE deterministic variant from `seed`, draws centered, returns false→caller keeps flat fill), `drawEnvDecor`, `envThemeForBiome`/`envThemeForDungeon`/`envDecorChance`, `Sprites.envHash(x,y,salt)` (stable per-tile hash → no flicker, no per-frame random). INTEGRATION: `engine.js renderTileMap` (world biome per-tile theme; nexus/vault use `tileMap.envTheme||'neutral'`) and `dungeon.js renderDungeonTiles` (theme from `defKey`) draw the env sprite AFTER the base color fill for floor/wall/hazard/water (+ sparse decor on plain floor/grass); the old lava-pulse/ice-shine/wall-stripe overlays now draw only when the env sprite did NOT draw (`!drewEnv`). Cells are FIRST-PASS guesses — retune col/row. Cull/camera/rotation/portal passes unchanged.
