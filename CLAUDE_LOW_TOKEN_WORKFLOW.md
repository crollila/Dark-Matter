...
- PORTAL SHEETS (SEPARATE system from mob/item): 10 atlases `portal_{void_arcane,blue_green,ice,void_dark,forest,infernal,plague,astral,fungal,cursed}` in SPRITE_SHEETS (1254x1254, `cols:8,rows:8`). `PORTAL_THEME_SHEET` (theme→sheet), `PORTAL_VARIANT` (theme→frame-group, all 0), `dungeonPortalTheme` (DUNGEONS key→theme, all 15 mapped). `Sprites.drawPortal(theme,cx,cy,size)` loops 3 frames (row0 cols 0,1,2, ~6.25fps) via `_drawSheetTile`; unknown theme→`magic`/blue_green; unloaded→false (rect fallback kept). Wired in `renderTileMap` (engine.js) for ALL portal tiles (WORLD/RAID/DUNGEON/VAULT): theme = `tileMap.portalThemeAt(tx,ty)` else `PORTAL_TILE_THEME[t]` default. world.js sets `map.portalThemeAt` (resolves dungeonPortals/pendingPortals→dungeonPortalTheme); dungeon exit portal uses default 'magic'. No portal art on non-portal entities.
